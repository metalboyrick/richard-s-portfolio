...
const ExperienceCardView = (props: ExperienceCardProps) => {
  ...
  return (
    <Flex direction="column" className={css.wrapper(isRightSided, isDarkTheme)}>
      <div className={css.role}>
        {role}{" "}
        <span className={css.company(isDarkTheme)}>{`@${convertToCamelCase(
          company
        )}`}</span>
      </div>
      ...
      <Flex direction="column">
        {description.map((item, index) => (
          ...
          >
            ...
            <div className={css.descText}>{item.toLowerCase()}</div>
          </Flex>
        ))}
      </Flex>
